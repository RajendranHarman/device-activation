 ...
 * It provides methods to insert, update, and retrieve device information from the database.
 */
@Repository
@Slf4j
public class DeviceInfoSharedDao {
    ...
    private static final String LOG_MESSAGE = " possibly not unique item, insertion attempted in DeviceInfo Table.";
    ...
    public int insert(final String harmanId, final String name, final String value) {
        ...
        if (updated == 0) {
            log.info(
                "Inside DeviceInfoDAO:insert() - 0 row affected, returning -1 to indicate failure, possibly not unique"
                    + " item, insertion attempted in DeviceInfo Table.");
            return RETURN_VALUE;
        }
        return updated;
    }
     ...
     *
     * @param harmanId the Harman ID
     * @param date the last login time
     */
    public void updateLastLoginTime(String harmanId, String date) {
        ...
        if (strLst.isEmpty()) {
            ...
            log.debug(
                "Inside DeviceInfoDAO:updateLastLoginTime(), more than one row exists for Lastlogintime"
                    + " for this HarmanID:- {}", harmanId);
    ...
    public long getOemIdOfDevice(String manufacturer, String hwSerialNum) {
        ...
        Long oemIdoOfDevice = jdbcTemplate.queryForObject(sql, new Object[]{manufacturer, hwSerialNum},
            new int[]{Types.VARCHAR, Types.VARCHAR}, Long.class);
        return oemIdoOfDevice != null ? oemIdoOfDevice : 0L;
    }
    ...
    /**
     * Deletes records from the DeviceInfo table based on the Harman ID.
     *
     * @param harmanId the Harman ID
    ...
    /**
     * Retrieves the device attributes to update.
     *
