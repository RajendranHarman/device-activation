 ...
 * preparing like and range queries, and preparing order by queries.
 */
@Slf4j
public abstract class SqlUtility {
    ...
    /**
     * Generates a prepared SQL statement based on the provided prefix, operator, and attribute-value map.
     *
     * @param prefix            the prefix to be added to the SQL statement
     * @param operator          the operator to be used in the SQL statement
     * @param attributeValueMap the map containing attribute-value pairs for the SQL statement
    ...
    /**
     * Converts the values of a given map into an array.
     *
     * @param orderedMap the map containing the values to be converted
     * @return an array containing the values from the map
     */
     ...
     *
     * @param containsLikeFieldList The list of fields to be used in the LIKE conditions.
     * @param containsLikeValueList The list of values to be used in the LIKE conditions.
    ...
    public static String prepareRangeQuery(List<String> rangeFieldList, List<String> rangeValueList) {
        ...
        for (int i = 0; i < rangeFieldList.size(); i++) {
            ...
            if (range.split(CommonConstants.UNDERSCORE).length != LENGTH) {
                log.warn("Inavalid range value can not be used in query:{}", range);
                continue;
            }
            String start = range.split(CommonConstants.UNDERSCORE)[0];
            String end = range.split(CommonConstants.UNDERSCORE)[1];
            if (stringQueryFilter == null) {
                stringQueryFilter = new StringBuilder();
            } else {
                stringQueryFilter.append(" and ");
    ...
    /**
     * Prepares an SQL query for ordering the result set based on the specified sorting order and column.
     *
     * @param sortingOrder The sorting order, either "asc" for ascending or "desc" for descending.
     * @param sortBy The column to sort by.
     * @return The prepared SQL query for ordering the result set.
     */
    public static String prepareOrderByQuery(String sortingOrder, String sortBy) {
        ...
        if (StringUtils.isNotEmpty(sortBy) && StringUtils.isNotEmpty(sortingOrder)
            ...
            queryOrderBy.append(" " + CommonConstants.ORDER_BY + "\"");
            ...
            queryOrderBy.append(" " + CommonConstants.ORDER_BY + "\" ID\" ASC ");
        }
    ...
    /**
     * Prepares the SQL query for sorting by a specific column and ordering the results.
     *
     * @param sortBy  the column to sort by
    ...
    /**
     * Prepares the SQL query for sorting and ordering the results based on the given parameters.
     *
     * @param sortBy  the column name to sort by
     * @param orderBy the order (ascending or descending)
     * @param table   the table name
     * @return the prepared SQL query for sorting and ordering
     */
    public static String prepareSortByAndOrderByQuery(String sortBy, String orderBy, String table) {
        StringBuilder queryOrderBy = new StringBuilder();

        if (StringUtils.isNotEmpty(sortBy) && StringUtils.isNotEmpty(orderBy)) {
            queryOrderBy.append(" " + CommonConstants.ORDER_BY + "\"" + table + "\"" + ".\"");
            queryOrderBy.append(sortBy);
            if ("asc".equalsIgnoreCase(orderBy)) {
                queryOrderBy.append("\" ASC ");
            } else {
