...
public class DeviceStateHistoryDataMapperTest {
    ...
    public void mapRowTest1() throws SQLException {
        String state = "ACTIVE";
        Mockito.doReturn(state).when(resultSet).getString("state");
        Mockito.doReturn("121212").when(resultSet).getString("imei");
        Mockito.doReturn("313131").when(resultSet).getString("serial_number");
        assertNotNull(deviceStateHistoryDataMapper.mapRow(resultSet, ROWNUM));
    }
